...
const CUE_HEADER_REGEXP = /((\d+:)?\d\d\:\d\d[.,]\d\d\d)\s-->\s((\d+:)?\d\d\:\d\d[.,]\d\d\d)/;

function parse (input) {

  let text = '';
  let checkCueHeader = false;
  let i = 0;
  const parts = [];
  ...
  // find the cues, based on empty line, followed by valid start/end time line
  for (i = 1; i < lines.length; i++) {
    if (lines[i].length === 0) {
      checkCueHeader = true;
      continue;
    }
    if (checkCueHeader && CUE_HEADER_REGEXP.test(lines[i])) {
      parts.push(text);
      text = '';
    }
    text += `${lines[i]}\n`;
    checkCueHeader = false;
  }
